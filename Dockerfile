...
WORKDIR /build/ecommerce-go-notification-service
...
COPY --from=builder /build/ecommerce-go-notification-service/main .

# Copy config files if they exist
COPY --from=builder /build/ecommerce-go-notification-service/config* ./config/ 2>/dev/null || true
